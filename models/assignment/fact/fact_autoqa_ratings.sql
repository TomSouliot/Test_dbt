...
autoqa_ratings as (
    select
        ...
        -- root cause metrics
        if(arca.nr_root_causes_provided>0,true,false) as has_root_cause_provided,
        arca.nr_root_causes_provided,
        arca.nr_distinct_root_causes,
        arca.root_causes_together
        

    from {{ ref('raw_autoqa_ratings') }} as ar
    left join auto_root_causes_agg as arca
        on ar.autoqa_rating_id = arc.autoqa_rating_id
)
